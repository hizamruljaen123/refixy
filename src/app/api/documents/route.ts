...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!title || !unit_id) {
      return NextResponse.json({ error: 'Title and unit are required' }, { status: 400 })
    }
    ...
    if (!ownerUser) {
      return NextResponse.json({ error: 'User not found in database. Please contact administrator.' }, { status: 400 })
    }
    ...
    const document = await db.document.create({
      data: {
        ...
        summary,
        category,
        classification: (classification as any) || 'LOW',
